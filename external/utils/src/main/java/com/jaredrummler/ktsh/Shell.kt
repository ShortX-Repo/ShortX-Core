...
class Shell @Throws(NotFoundException::class) @JvmOverloads constructor(
    ...
    @Synchronized
    fun shutdown() {
        Log.w("TORNACO.Shell", "shutdown")
        try {
            write("exit")
            Log.w("TORNACO.Shell", "Call waitFor")
            process.waitFor()
            Log.w("TORNACO.Shell", "Call closeQuietly")
            stdin.closeQuietly()
            Log.w("TORNACO.Shell", "Call clear")
            onStdOutListeners.clear()
            onStdErrListeners.clear()
            Log.w("TORNACO.Shell", "Call join")
            stdoutReader.join()
            stderrReader.join()
            process.destroy()
            Log.w("TORNACO.Shell", "Shell destroy!")
    ...
    fun forceShutdown() {
        Log.w("TORNACO.Shell", "forceShutdown")
        try {
            onStdOutListeners.clear()
            onStdErrListeners.clear()
            process.destroy()
    ...
    private class StreamReader private constructor(
        ...
        override fun run() {
            runCatching {
                ...
                Log.w("Tornaco.Shell[BufferedReader]", it.stackTraceToString())
            }
        }
    ...
    companion object {
        ...
        val SU: Shell get() = this["su --mount-master"]
